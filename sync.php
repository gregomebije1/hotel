...
if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'Begin Synchronization')) {
  $result1 = mysql_query("select * from sync_settings", $con);
  if (mysql_num_rows($result1) <= 0) {
    echo msg_box("Your sync settings for connecting with external database is empty", 'sync_settings.php', 'Back to sync settings');
	exit;
  }
  $row = mysql_fetch_array($result1);
  if (!file_exists($_REQUEST['file'])) {
    echo msg_box("The file {$_REQUEST['file']} does not exist
  ...
  <table>
   <tr class='class1'>
      ...
      <tr>
       ...
       <td><input type="text" name="s_host" size="50" value="<?=$row['s_host']?>" disabled="disabled"></td>
      </tr>
      <tr>
       <td>path information</td>
       <td><input type="text" name="s_path" size="50" value="<?=$row['s_path']?>" disabled="disabled"></td>
      </tr>
       <td>Username</td>
	   <td><input type="text" name="s_username" size="50" value="<?=$row['s_username']?>" disabled="disabled"></td>
      </tr>
      <tr>
       <td>Password</td>
	   <td><input type="password" name="s_password1" size="50" value="<?=$row['s_password']?>" disabled="disabled"></td>
      </tr>
      <tr>
       <td>Re-nter Password</td>
	   <td><input type="password" name="s_password2" size="50" value="<?=$row['s_password']?>" disabled="disabled"> </td>
      </tr>
	  <tr>
       <td>Backup File</td>
